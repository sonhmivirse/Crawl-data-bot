...
class VietducCrawler(TextCrawler):
    ...
    def crawl_url(self, source, summary="", title="", category=""):
        ...
        try: 
            ...
            title = soup.select_one("div.news-primary-content h1")
            title = title.get_text(strip=True) if title else ""
                        
            text = soup.select_one("div.news-primary-content")
            
            if text:
                for unwanted_class in ["news-detail-bot" ,"news-new-hot-content"]:
                    for element in text.select(f"div.{unwanted_class}"):
                        element.decompose()
                div_clear = text.find_next("div", class_="clear")
                if div_clear:
                    table = div_clear.find_previous("table")
                    if table:
                        table.decompose()
                        
            text = str(text) if text else ""
            text = text.replace("Bệnh viện Hữu nghị Việt Đức là địa chỉ thăm khám, phòng ngừa và điều trị các bệnh lý về Tim mạch và Lồng ngực uy tín hàng đầu. Để tìm hiểu thêm về thông tin bệnh lý và nhu cầu chăm sóc sức khỏe vui lòng liên hệ:", "")
            ...
            with open("error_vietduc.txt", "a") as f:
                f.write(f"\n{source}")
            
    
    def crawl_source(self, source):
        """Crawl from a parent list source (include pages)"""
        root_url = "https://benhvienvietduc.org"
        ...
        for page in tqdm(range(start_page, num_page + 1), desc="Crawl page", unit="page"):
            try:
                ...
                list_items = soup.select("div.news-primary-content li a")
                ...
                with open("Error_page_vietduc.txt", "a") as f:
