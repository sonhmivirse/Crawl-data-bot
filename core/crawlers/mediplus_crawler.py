...
class MediplusCrawler(TextCrawler):
    ...
    def crawl_url(self, source, summary="", title="", category=""):
        ...
        try: 
            ...
            title = soup.select_one("div.blogDetail__cont div.blogDetail__title h1")
            title = title.get_text(strip=True) if title else ""
            
            text = soup.select_one("div.blogDetail__cont div.blogDetail__info")
            
            if text:
                for unwanted_class in ["wpcf7"]:
            ...
            with open("error_mediplus.txt", "a") as f:
                f.write(f"\n{source}")
            
    
    def crawl_source(self, source):
        """Crawl from a parent list source (include pages)"""
        root_url = "https://www.mediplus.vn"
        ...
        for page in tqdm(range(start_page, num_page + 1), desc="Crawl page", unit="page"):
            try:
                ...
                list_items = soup.select("div.listBlogVertical__row div.handBook__item a")
                ...
                with open("Error_page_mediplus.txt", "a") as f:
